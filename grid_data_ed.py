...
def simpson_weights(n, a=0.0, b=2 * np.pi):
    
    if n % 2 == 0:  
        n += 1
    h = (b - a) / (n - 1)  
    w = np.full(n, 4 * h / 3)  
    w[1::2] = 2 * h / 3  # Weight for odd indices (except endpoints)
    w[0] = w[-1] = h / 3  # Endpoints
    return w
...
class grid_data:
    def __init__(self, m, bdry, l, precond=True, order='cubic', density=1, bc='dirichlet', nr_quad=18, ntheta_quad=50):
        ...
        self.ntheta_quad = ntheta_quad if ntheta_quad % 2 == 1 else ntheta_quad + 1
        ...
        self.w_theta_quad = simpson_weights(self.ntheta_quad)
        ...
        if precond:
            ...
            self.distancex = np.absolute(np.outer(np.ones(self.p), self.bn[:, 0]) - np.outer(self.bn[:, 0], np.ones(self.p)))
            self.distancey = np.absolute(np.outer(np.ones(self.p), self.bn[:, 1]) - np.outer(self.bn[:, 1], np.ones(self.p)))
    ...
    def createflag(self, f):
        ...
        boundary_r_sq = f[0](theta)**2 + f[1](theta)**2
    ...
    def FD(self, w1):
        ...
        v1[self.flag] = w1
        v = self.m**2 / (2 * np.pi)**2 * (self.flag2 * v1 - np.roll(v1, -1, 0) - np.roll(v1, 1, 0) - np.roll(v1, -1, 1) - np.roll(v1, 1, 1))
        return v[self.flag]
